...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in livingGhostPositionDistributions:
            position = i.argMax()
            if closest > self.distancer.getDistance(pacmanPosition, position):
                target = position
                closest = self.distancer.getDistance(pacmanPosition, position)
        
        newDistance = []
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            newDistance.append((self.distancer.getDistance(successorPosition, target), action))
        return min(newDistance)[1]

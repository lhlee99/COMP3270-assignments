...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if not actions:
          ...
          tmp = []
          for a in actions:
            tmp.append(self.getQValue(state, a))
          return max(tmp)  
    ...
    def computeActionFromQValues(self, state):
        ...
        if not actions:
          ...
          maxs = []
          for a in actions:
            tmp.append((self.getQValue(state, a), a))
          greatest = max(tmp)
          tmp.sort()
          while greatest in tmp:
            maxs.append(tmp.pop(-1))
          return random.choice(maxs)[1]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        # print("print:")
        # print(self.featExtractor.getFeatures(state, action)[state, action])
    ...
    def update(self, state, action, nextState, reward):
        ...
        for key in self.featExtractor.getFeatures(state, action).keys():
          self.getWeights()[key] += self.alpha * difference * self.featExtractor.getFeatures(state, action)[key]
